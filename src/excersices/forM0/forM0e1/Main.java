package excersices.forM0.forM0e1;

import excersices.forM0.forM0e1.victims.RunnableCaller;
import excersices.forM0.forM0e1.victims.SumRunnable;
import excersices.forM0.forM0e1.victims.SumThread;
import excersices.forM0.forM0e1.victims.ThreadCaller;
...
//		}
        ...
        if (actualResult != p.sum) {
            throw new RuntimeException(String.format("excersices.forM0.forM0e2.victims.SumThread calculates" +
    ...
    private static void testComputingInSumRunnable(RandomPair pair) {
        ...
        if (actualResult != pair.sum) {
            throw new RuntimeException(String.format("excersices.forM0.forM0e2.victims.SumRunnable calculates" +
                            "incorrect sum for the input: %s," +
                            " expected result: %d, actual" +
                            " result: %d",
                    Arrays.toString(pair.array),
